...
-- AI Analyses table (standalone, no foreign key to support_tickets)
CREATE TABLE IF NOT EXISTS ai_analyses (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  ticket_id UUID, -- Remove foreign key constraint for now
...
-- AI analyses policies (allow all authenticated users for now)
...
-- AI fixes policies (allow all authenticated users for now)
CREATE POLICY "Authenticated users can view fixes"
  ON ai_fixes FOR SELECT
  USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can manage fixes"
  ON ai_fixes FOR ALL
  USING (auth.uid() IS NOT NULL);