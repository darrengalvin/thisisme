...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get the current user using cookie-based auth (same as other working APIs)
    const { data: { user }, error: authError } = await supabase.auth.getUser()
    
    console.log('🔍 UPGRADE API: Auth check result:', { 
      hasUser: !!user, 
      userId: user?.id, 
      email: user?.email,
    ...
    console.log('🔄 UPGRADE API: Processing upgrade for user:', user.id, user.email)
