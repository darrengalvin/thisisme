...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🎤 Webhook URL:', `${process.env.NEXT_PUBLIC_BASE_URL || 'https://thisisme-m2ku5utm7-darrengalvins-projects.vercel.app'}/api/vapi/webhook?userId=${user.id}&secret=${process.env.VAPI_WEBHOOK_SECRET || 'dev-secret-123'}`)
    ...
    return NextResponse.json({
      ...
      instructions: {
        ...
        webhookUrl: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://thisisme-m2ku5utm7-darrengalvins-projects.vercel.app'}/api/vapi/webhook?userId=${user.id}&secret=${process.env.VAPI_WEBHOOK_SECRET || 'dev-secret-123'}`,
