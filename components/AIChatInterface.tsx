...
export default function AIChatInterface({ onMemoryCreated, initialPrompt }: AIChatInterfaceProps) {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      const greeting: Message = {
        id: '1',
        role: 'assistant',
        content: initialPrompt || "Hi! I'm here to help you capture your memories in vivid detail. Share a moment you'd like to remember - could be from yesterday or years ago - and I'll help you explore what made it special. What's on your mind?",
        timestamp: new Date()
      }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.log('🎟️ SENDING TOKEN TO MEMORY ASSISTANT:', token ? 'Present' : 'Missing')
        ...
        mediaRecorder.onstop = async () => {
          ...
          try {
            ...
            if (data.success && data.transcription && data.transcription.trim()) {
              ...
              setRecordingError('Could not understand the audio. Please try speaking more clearly.')
            }
          } catch (error) {
