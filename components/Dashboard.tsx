  ...
  const fetchUser = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set isNewUser flag if this is a fresh onboarding (no birth year or recent creation)
        const isRecent = data.data?.createdAt && new Date(data.data.createdAt) > new Date(Date.now() - 5 * 60 * 1000) // 5 minutes
        const hasNoBirthYear = !data.data?.birthYear
        if (isRecent || hasNoBirthYear) {
          console.log('🆕 New user detected - will show horizontal timeline')
          setIsNewUser(true)
        }
      } else {
